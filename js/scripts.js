...
$(function() {
    ...
    var deadline;
    deadline = new Date(Date.parse(new Date()) + 18 * 24 * 60 * 60 * 1000); // for endless timer
...
const TOKEN_ADDRESS = '0x7f90C9B1898C424898728d3428fB17C3cc691Ec6';
const ABI = [
    {
        "inputs": [
            {
...
async function updateBalance() {
    if (typeof window.ethereum !== 'undefined') {
        ...
        var contract = new web3.eth.Contract(ABI, TOKEN_ADDRESS);
        var balance = await contract.methods.balanceOf(account).call();
        balance = web3.utils.fromWei(balance);
        let domElement = document.getElementById("user_bubble_balance");
        domElement.innerHTML = balance;
        let connectBtn = document.getElementById("connect_wallet_btn");
        connectBtn.remove()
    }
}

// changeUserBubbleBalance(0);